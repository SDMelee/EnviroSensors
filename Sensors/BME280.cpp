...
BME280::BME280(int sda, int scl, String mode){
	_sda=sda;
	_scl=scl;
	_mode=mode;	
}
...
float BME280::readTemp(String units){
	if(units=="C")
		return readTempC();
	else if(units=="F")
		return readTempF();
...
void BME280::printAll(){
	...
	Serial.print(readFloatPressure(), 2);
	Serial.println(" Pa");

	Serial.print("Altitude: ");
	Serial.print(readFloatAltitudeMeters(), 2);
	Serial.println("m");

	Serial.print("Altitude: ");
	Serial.print(readFloatAltitudeFeet(), 2);
	Serial.println("ft");	
